...
bool AddrSpace::usedPhysicalPage[NumPhysPages] = {false};
...
bool AddrSpace::Load(char *fileName) {
  ...
  // check to be used memory not larger than remains
  ...
  for (int i = 0; i < numPages; i++) {
    ...
    while (j < NumPhysPages && AddrSpace::usedPhysicalPage[j])
      j++;
  ...
  if (noffH.code.size > 0) {
    ...
    executable->ReadAt(
        &(kernel->machine->mainMemory
              [pageTable[noffH.code.virtualAddr / PageSize].physicalPage *
                   PageSize +
  ...
  if (noffH.initData.size > 0) {
    ...
    executable->ReadAt(
        &(kernel->machine->mainMemory[pageTable[noffH.initData.size / PageSize]
                                              .physicalPage *
                                          PageSize +
                                      noffH.initData.size % PageSize]),
  ...
  if (noffH.readonlyData.size > 0) {
    ...
    executable->ReadAt(
        &(kernel->machine
              ->mainMemory[pageTable[noffH.readonlyData.virtualAddr / PageSize]
                                   .physicalPage *
                               PageSize +
